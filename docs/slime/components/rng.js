...
AFRAME.registerComponent('rng-building-shader', {
  ...
  tick: function () {
    if (window.SLIME_PAUSED) return;
    var data = this.data;
    if (!this.cam) { this.cam = document.querySelector('#rig'); }
    if (this.cam && data.fog) {
      var campos = this.cam.getAttribute('position');
      if (!this._worldPos) { this._worldPos = new THREE.Vector3(); }
      var pos = this.el.object3D.getWorldPosition(this._worldPos);
      var size = this.el.children.length;
      var check = this.el.children[size-1].getObject3D('mesh').material.uniforms
      if (check) {
        var dx = pos.x - campos.x, dy = pos.y - campos.y, dz = pos.z - campos.z;
        this.el.children[size-1].getObject3D('mesh').material.uniforms['camdist']['value'] = Math.sqrt(dx*dx + dy*dy + dz*dz);
      }
    }
  }
});
