...
AFRAME.registerShader('starfield-shader', {
  schema: {
    timeMsec: {type: 'time', is: 'uniform'},
    density: {type: 'float', is: 'uniform', default: 80.0},
    flickerSpeed: {type: 'float', is: 'uniform', default: 3.0},
    color1: {type: 'color', is: 'uniform', default: '#000000'},
    color2: {type: 'color', is: 'uniform', default: '#000000'},
    color: {type: 'color', is: 'uniform', default: '#ffffcc'},
  },
  vertexShader: basic,
  fragmentShader: `
varying vec2 vUv;
...
void main() {
  ...
  vec2 starPos = vec2(hash(cell), hash(cell + 100.0));
  float d = length(local - starPos);

  float starChance = hash(cell + 300.0);
  float size = 0.02 + 0.03 * hash(cell + 400.0);
  float star = smoothstep(size, 0.0, d) * step(0.65, starChance);

  float vol = color1.x;
  float t = timeMsec * 0.001 * flickerSpeed;
  float timeFlicker = 0.6 + 0.4 * sin(t * (0.5 + hash(cell + 200.0) * 4.0) + hash(cell + 500.0) * 6.28);
  float audioFlicker = 0.3 + vol * (0.5 + 0.5 * sin(hash(cell + 200.0) * 6.28));
  float flicker = mix(audioFlicker, timeFlicker, step(0.99, vol));
  star *= flicker;

  float warmth = hash(cell + 600.0);
  vec3 tint = mix(color, vec3(1.0), warmth * 0.3);

  gl_FragColor = vec4(tint * star, 1.0);
}
`
});
